...
\lstdefinelanguage{inslibyaml}{
  ...
  morekeywords={name,aiding,init,automotive_mode,automotive_min_speed_mps,
    ...
    enable,stddev_ut,min_delay_ms,wmm_year,estimate_bias,bias_init_ut,bias_rw_ut_sqrts,
    fixed_bias,misalignment,stddev_m,acc_bias_rw,
    ...
    lim_ars_yaw_drift_deg_min,coast_gap_min_sec,lim_coast_exit_err_m,ref_delay_ms,
    automotive_lateral_constraint,automotive_lateral_stddev_mps,
    automotive_lateral_max_yaw_rate_deg,automotive_lateral_after_sec,inputs},
...
\newcommand{\inslibversion}{1.1.0}
\author{Jan Zwiener \\ \texttt{jan@zwiener.org}}
\date{Version \inslibversion\\[2mm] \today}
...
\begin{itemize}
  ...
  \item \textbf{Single-precision hot path.} Strapdown, prediction and
        ...
        absolute-position anchor (the geodetic origin, the
        latitude/longitude/height bookkeeping and the geodetic differences
        formed against them), touched only at initialisation, in the GNSS
        residual and in accessors.
...
\end{tabular}
\end{center}
The absolute position is anchored in double precision, geodetically in
both places: the n-frame origin as $(\varphi,\lambda,h)_{\text{origin}}$,
and the current latitude/longitude/height $(\varphi,\lambda,h)$ bookkept
incrementally from it (\S\ref{sec:llh}).
...
\begin{equation}
  ...
  \Delta h = -\Delta p_D .
\end{equation}
The curvature terms (and gravity) change only by
$\sim d / R_{\text{earth}}$ per metre of travel and are cached,
refreshed after every \SI{100}{\metre} of accumulated travel. $\Rne$ is
deliberately not among them: no epoch-path computation uses it, so it is
built on demand in \code{ins\_\allowbreak{}get\_\allowbreak{}velocity\_\allowbreak{}ecef()}
rather than refreshed inside the worst-case epoch. This keeps
...
The measurement arrives as geodetic $(\varphi,\lambda,h)$ in double
precision, the form receivers report, with a full $3\times 3$ NED
covariance. No coordinate conversion runs on the epoch path: the fix is
differenced against the filter's anchor at the measurement's time of
validity directly, and the difference is mapped to metres with the
curvature radii. A source that is natively ECEF (RTK, UBX-NAV-HPPOSECEF)
converts once in the caller, outside the filter's worst case. With the
...
The scale error of the vehicle's speed signal does: rolling radius, tyre
wear and the scaling the ECU applies are a few percent, which at the
library's default \SI{3}{\percent} is \SI{0.83}{\metre\per\second} at
\SI{100}{\kilo\metre\per\hour} -- ten times the quantisation noise and
\emph{systematic}. Its sign is not predictable from the type-approval
margin that keeps an indicated speed from ever falling below the true
one: that constrains the dashboard, whereas \code{PID 0x0D} is the ECU's
own value and a different number from the one the cluster shows. A
constant scale factor removes the calibrated part and a relative variance
prices what is left:
\begin{equation}
...
\code{tools/inslib\_\allowbreak{}speed\_\allowbreak{}scale.py} fits
$s_{\text{scale}}$ against GNSS speed and prints the \code{config.yaml}
value.
...
\paragraph{Tuning the hard-iron states.} Two parameters govern them:
\code{mag\_bias\_init\_stddev\_ut} (\SI{10}{\micro\tesla} by default),
the prior on how much hard iron the filter should be prepared to find,
and \code{mag\_bias\_pred\_stddev\_ut\_sqrts}
(\SI{0.129}{\micro\tesla\per\sqrt\second} by default, which is
\SI{1}{\micro\tesla} per minute), the random walk. Neither is right
everywhere. The initial sigma is the stronger lever: a recording whose
magnetometer was calibrated where it sits wants a tight prior, while a
unit just installed in an unknown vehicle does not, since
\SIrange{10}{20}{\micro\tesla} of vehicle hard iron is ordinary. The
random walk is better read as how long the estimate keeps listening
after it has settled than as a physical drift rate: hard iron steps
when ferrous mass moves rather than wandering continuously, and what
the number decides is whether such a step is still followed. Against a
per-axis sigma of a few \si{\micro\tesla} fused at about \SI{1}{\hertz}
the state settles near $(q^2\,\dt\,R)^{1/4}$ and admits $q\sqrt{T}$ of
drift over a time $T$; too small a value converges to a confidence it
then cannot be talked out of. Both are exposed to the replay harness as
\code{mag.bias\_init\_ut} and \code{mag.bias\_rw\_ut\_sqrts}.
...
reference field is unusable, either because the reference vector itself
is degenerate or because the position lies inside a dip pole exclusion
zone (\S\ref{sec:wmm-dipzone}).
...
\begin{itemize}
  ...
  \item \textbf{Position/velocity} from the fix; with a local-position
        fix the caller's geodetic anchor is kept as origin and the fix sets
...
along the local down axis without changing the absolute solution. Down is
the ellipsoid normal, which is the axis the height is measured along, so
the shift touches the origin height alone: $h_{\text{origin}}$ decreases by
$\Delta z$ while $\varphi_{\text{origin}}$ and $\lambda_{\text{origin}}$ stay
bit-for-bit unchanged, \code{pos\_\allowbreak{}local[2]} (state and history) shifts by
...
field. The latter is a measurement-side guard and only bites within
about a degree of a dip pole, so it is backed by the position-side
exclusion zone (\S\ref{sec:wmm-dipzone}), which withdraws the heading
reference over the surrounding region where the declination is already
unusable.
...
The step is only sound while the yaw is \emph{magnetically anchored},
which is what makes the rotation a known one. Inside a dip pole
exclusion zone (\S\ref{sec:wmm-dipzone}) the AHRS suspends magnetometer
yaw fusion and keeps its previous declination, so the yaw is carried by
the gyro instead. On the position that leaves the zone the new
declination is therefore adopted \emph{without} re-framing the
attitude, and the fusion pulls the accumulated drift out through the
ordinary covariance weighting. Re-framing there would be actively
harmful: the declination on the far side of a dip pole differs by tens
of degrees (about $80^\circ$ for a typical crossing), and applying that
as a rotation would corrupt an otherwise sound estimate rather than
correct it. Roll and pitch are unaffected throughout, only the heading
reference is withdrawn.

The one case with no previous declination to keep is a first position
that already lies inside a zone, after the yaw has been fused against
magnetic north. That yaw is a magnetic heading, and no re-framing will
ever make it a true one, so the AHRS widens its yaw variance to
``heading unknown'' there instead of letting the covariance of that
fusion stand. While the attitude-precision restart watchdog is armed the
widened value stays just below its yaw threshold, since a restart would
discard the zone state together with the position. Otherwise the nav\_suite attitude hint would hand the
magnetic heading to ins as a converged true heading. A yaw that was
never fused against the magnetometer keeps its variance.
...
lat/lon/ellipsoid-height. Both directions use the same curvature-radii
mapping $\mathcal{M}$ that the filter moves its own anchor with
(\S\ref{sec:llh}), applied around the geodetic origin
$(\varphi,\lambda,h)_{\text{origin}}$:
\begin{equation}
  (\varphi,\lambda,h) = (\varphi,\lambda,h)_{\text{origin}}
    + \mathcal{M}^{-1}\!\left(\vect{p}_{\text{local}}^{\,n}\right),
  \qquad
  \vect{p}_{\text{local}}^{\,n} = \mathcal{M}\!\left(
    (\varphi,\lambda,h) - (\varphi,\lambda,h)_{\text{origin}} \right).
\end{equation}
There is deliberately no detour through ECEF here: routing it that way
would answer the same question with a different approximation than the
filter's own position bookkeeping uses, so the local frame and the
absolute solution would disagree at the metre level far from the origin.
This is the GNSS-side counterpart of the local-position offset: it turns
the local solution into WGS84 for telemetry, or WGS84 waypoints into the
frame a controller flies in.
...
\subsection{Declination is a wrapped angle}
\label{sec:wmm-wrap}

$D$ lives in $(-180^\circ, +180^\circ]$, so two neighbouring grid nodes
can sit on opposite sides of that cut while being a few degrees apart
(for example $-176.5^\circ$ and $+178.5^\circ$). Interpolating the
tabulated numbers directly would then run the long way round and return
a value off by up to $180^\circ$. Both the spatial and the temporal
interpolation therefore put the operands on a common branch first: the
secular change per node is taken the short way, and the three
neighbouring nodes are shifted next to the first before the bilinear
blend. The affected cells lie along the agonic line trailing each dip
pole, which is where the filters are most in need of a trustworthy
heading reference.

\subsection{Dip pole exclusion zones}
\label{sec:wmm-dipzone}

At a magnetic dip pole the horizontal field vanishes, so $D$ is not
merely inaccurate there but ill-conditioned: it rotates through the full
circle around the pole, and no regular grid resolves that. The exact
model puts the residual interpolation error at roughly $179^\circ$
within a degree of a pole; outside the $7^\circ$ exclusion radius the
worst case is about $10^\circ$, for $0.75\,\%$ of the Earth's surface. A
magnetometer is no better off, because the horizontal component it needs
has fallen to a few percent of the total field.

\code{magnetic\_\allowbreak{}dip\_\allowbreak{}pole\_\allowbreak{}distance\_\allowbreak{}deg()} reports the
great-circle distance to the nearest dip pole and
\code{magnetic\_\allowbreak{}heading\_\allowbreak{}reference\_\allowbreak{}valid()} is false within
\code{MAGNETIC\_\allowbreak{}DIP\_\allowbreak{}POLE\_\allowbreak{}EXCLUSION\_\allowbreak{}DEG} of one. Inclination
and total field strength stay usable inside a zone, only the heading
reference is withdrawn. Three properties are worth stating explicitly:

\begin{description}
  \item[Great circle, not latitude and longitude.] Near the northern
    pole a degree of longitude covers about a twelfth of the ground a
    degree of latitude does ($46$\,km against $555$\,km at
    $85^\circ$\,N), so a box in coordinates would be wrong by an order
    of magnitude in one direction.
  \item[Not a latitude limit.] The southern dip pole sits near
    $64^\circ$\,S, far from the geographic pole, so no cutoff on
    latitude separates the zones from ordinary terrain without
    discarding the whole subantarctic.
  \item[The count is data, not an invariant.] The number of dip poles
    and their positions are measured per epoch by the generator and
    tabulated (\code{WMM\_\allowbreak{}DIP\_\allowbreak{}POLE\_\allowbreak{}COUNT}), and the runtime
    walks whatever it finds. Today's field has two, both drifting, the
    northern one by roughly $33$\,km per year. That is a property of the
    present dipole dominance rather than something the code may assume.
  \item[No interpolation in time.] Positions are tabulated once at
    mid-epoch and the query takes no year, unlike the declination. Over
    five years the poles move about $1.5^\circ$, so a fixed mid-epoch
    position is at most $\sim0.8^\circ$ off at either end -- smaller
    than the slack in the radius, and smaller than the uncertainty in
    picking that radius in the first place. Interpolating would compute
    a correction finer than the quantity it feeds. The generator
    measures the drift on every regeneration and refuses to emit
    positions that exceed the budgeted margin.
\end{description}

Because the poles move, the exclusion radius wants re-examining whenever
the tables are regenerated. \code{magneticmodel/wmm\_\allowbreak{}error\_\allowbreak{}analysis.py}
re-measures the shipped table against the exact model and reports the
residual error outside the zones for exactly that purpose. Sample it at
$0.5^\circ$ or finer: the worst points sit on a thin ridge at the
exclusion boundary, and a coarser grid steps over them and understates
the maximum.
...
\begin{description}
  ...
  \item[Deterministic declination step.] Referencing to true north is a
    ...
    covariance). It is skipped on leaving a dip pole exclusion zone,
    where the yaw was carried by the gyro rather than the field and the
    rotation would therefore not be a known one
    (\S\ref{sec:wmm-dipzone}).
  \item[Dip pole zones are excluded by distance, not by latitude.] The
    southern dip pole sits near $64^\circ$\,S, so a latitude cutoff
    cannot separate the degenerate region from ordinary terrain. The
    number of poles and their drifting positions are measured per epoch
    and tabulated rather than assumed in code, because both are
    properties of the present field and not invariants.
...
\code{baro.csv}, \code{speed.csv}, the column contract lives in
\code{datasets/replay\_\allowbreak{}format.py})
...
mag:
  ...
  bias_init_ut: 0.0         # INITIAL hard-iron stddev [uT], 0 -> default
  bias_rw_ut_sqrts: 0.0     # hard-iron random walk [uT/sqrt(s)], 0 -> default
...
\begin{description}\setlength\itemsep{0pt}
  ...
  \item[\code{mag: wmm\_\allowbreak{}year}] \emph{decimal year.} Epoch for the World Magnetic
    Model declination lookup. For the live receiver (\code{insrcv}) it is
    optional: without it the epoch comes from GPS time in the stream,
    failing that from the UTC date the receiver reports in NAV-PVT, and
    failing that from the host clock. The replay harnesses have no date
    source of their own, so there it is required whenever \code{mag: enable}
    is set: without it no reference field is built, the magnetometer is
    not fused, and the harness warns once at start-up.
    \code{tools/inslib\_\allowbreak{}convert\_\allowbreak{}ubx\_\allowbreak{}to\_\allowbreak{}csv.py} fills it in
    from the capture's NAV-PVT date.
...
\paragraph{Non-holonomic lateral constraint (top level, optional).}
\code{automotive\_\allowbreak{}lateral\_\allowbreak{}constraint} [0/1, default 0] fuses a synthetic
measurement stating that a wheeled vehicle does not travel sideways: the
body-frame lateral velocity against a truth of zero (REQ-NAV-077). It needs
\code{automotive\_\allowbreak{}mode} and shares its \code{automotive\_\allowbreak{}min\_\allowbreak{}speed\_\allowbreak{}mps}.
What the constraint is for is its attitude row, whose gain is the ground
speed: at 20\,m/s one degree of roll error is 0.35\,m/s of lateral body
velocity, and during a GNSS outage that channel is otherwise unobserved.
\code{automotive\_\allowbreak{}lateral\_\allowbreak{}stddev\_\allowbreak{}mps} [m/s] is the
measurement noise, \code{automotive\_\allowbreak{}lateral\_\allowbreak{}max\_\allowbreak{}yaw\_\allowbreak{}rate\_\allowbreak{}deg}
[deg/s] skips the fusion in turns where side slip breaks the
assumption, and \code{automotive\_\allowbreak{}lateral\_\allowbreak{}after\_\allowbreak{}sec} [s] holds it off until the filter has been that long
without a GNSS fusion.
...
\code{ref\_\allowbreak{}delay\_\allowbreak{}ms} [ms, default 0] states that a reference row
timestamped $T$ describes the state at $T$ minus that delay, and the harness
scores every comparison at that time of validity (REQ-VER-030). Set it to
mirror \code{gnss: delay\_\allowbreak{}ms} whenever \code{ref.csv} comes out of the same
receiver output as the aiding: \code{gnss: delay\_\allowbreak{}ms} tells the filter how old
a fix is so that it reports where the vehicle is NOW, while the reference row
still describes where it was, so comparing both at a shared timestamp
penalises the filter by speed times delay --- and penalises it more the
better it compensates. Leave it at 0 for an independent reference (Lighthouse,
synthetic truth) and for a dataset that models the latency in its data rather
than its config. A nonzero value with \code{aiding: ref} is refused, since
there the reference is the aiding.

\code{coast\_\allowbreak{}gap\_\allowbreak{}min\_\allowbreak{}sec} [s, 0 $	o$ off] turns on the
coasting re-acquisition metric (REQ-VER-029): every gap in the aiding file
longer than it is reported with its duration, the straight-line distance
across it and the filter's position error at the first IMU epoch at or after
its end, scored against the reference epoch nearest to that IMU epoch (none
further than half a second, or half the gap, away), and \code{lim\_\allowbreak{}coast\_\allowbreak{}exit\_\allowbreak{}err\_\allowbreak{}m} [m, 0 $	o$ not
gated] is the guard-rail on that error. This is the metric for a dataset with
a real outage, where the whole-run \code{lim\_\allowbreak{}pos\_\allowbreak{}rms\_\allowbreak{}m} cannot
answer the question: no fixes means no reference inside the gap, and a filter
that gives the coasting solution up scores BETTER on the RMS than one that
coasts through, because it contributes no epochs where the error is large.
The error is sampled before that epoch's measurements are fused --- the
returning fix snaps the position onto the truth in the same epoch it arrives,
so a sample taken afterwards reports how good the fix was rather than how far
the coasting drifted. Having no solution when aiding returns counts as a
failure of this gate rather than as a skipped epoch. The gaps are taken from the aiding file,
not from what the filter chose to fuse, so the number stays comparable across
configurations that gate fixes differently.
...
\code{gnss.csv}, \code{mag.csv}, \code{baro.csv} and \code{speed.csv} are
consumed only when the matching \code{aiding}/sensor option is on. The
first line of each file is a \code{\#} header, every column layout:
...
# speed.csv  (optional, speed: enable) -- any further columns are the
#            producer's own record, both harnesses read the first two
  t_us, speed_mps
\end{lstlisting}
...
\begin{itemize}
  ...
  \item \textbf{Stack usage} (\code{make stack}): compiles the library with
    GCC's \code{-fcallgraph-info=su}, which reports the frame size of every
    function and the call graph after inlining, and computes the worst-case
    stack of every public API function along its deepest call chain
    (\code{scripts/stack\_usage.py}). It fails on recursion, variable length
    arrays, calls through function pointers or into library functions that
    \code{scripts/stack\_usage.cfg} does not give a figure for, and on a
    worst case above its budget in that file. The budgets there are for an
    x86\_64 host. For a target, run the script with its cross compiler and
    build flags and a config holding the figures of its C library: the
    script reads several config files and, given the stack a program
    reserves, checks the main task plus nested interrupt handlers and
    exception frames against it. Frame sizes depend on the architecture and
    the optimization level, so a host figure is no substitute for the
    target's.
